...
export default function RegisterPage() {
  ...
  const form = useForm<RegisterInput>({
    ...
    defaultValues: { email: "", password: "", displayName: "", role: "student" },
  });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-12 bg-background overflow-hidden relative">
      ...
      {isTeacher && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 fade-in">
          <div className="flex items-center gap-2 bg-teal-500/15 border border-teal-500/30 text-teal-400 rounded-full px-5 py-2 text-sm font-medium shadow-lg">
            ...
            Teachers get full access to the School World — we'll set it up next!
          </div>
        </div>
      )}
      ...
      >
        <div className={`bg-card border rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 ${isTeacher ? "border-teal-500/40 shadow-teal-500/10" : "border-border"}`}>
          <div className={`h-1 w-full bg-gradient-to-r transition-all duration-500 ${isTeacher ? "from-teal-400 via-cyan-400 to-emerald-400" : "from-primary via-cyan-400 to-purple-500"}`} />

          <div className="p-8">
            {/* Logo */}
            <div className="text-center mb-8">
              <Link href="/" className="inline-flex items-center justify-center gap-2 mb-4 group">
                <div className={`flex h-11 w-11 items-center justify-center rounded-xl transition-all group-hover:scale-105 ${isTeacher ? "bg-teal-500" : "bg-primary"}`}>
                  {isTeacher ? <GraduationCap className="h-5 w-5 text-white" /> : <TrendingUp className="h-5 w-5 text-primary-foreground" />}
                </div>
                <span className="text-2xl font-bold">12Digits</span>
              </Link>
              <h1 className="text-2xl font-bold text-foreground">
                {isTeacher ? "Create Teacher Account" : "Create your account"}
              </h1>
              <p className="text-muted-foreground text-sm mt-1">
                {isTeacher ? "Joining thousands of educators" : "Join thousands learning to trade"}
              </p>
            </div>
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                          <FormControl>
                            <SelectTrigger data-testid="select-role" className={`transition-all ${isTeacher ? "border-teal-500/50 focus:border-teal-500" : ""}`}>
                ...
                {watchRole === "student" && (
                  <div className={`space-y-4 transition-all duration-400 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`} style={{ transitionDelay: "360ms" }}>
                    <FormField
                      control={form.control}
                      name="schoolId"
                      render={({ field }) => (
                ...
                <Button
                  type="submit"
                  className={`w-full font-semibold transition-all hover:-translate-y-0.5 hover:shadow-lg ${isTeacher ? "bg-gradient-to-r from-teal-500 to-cyan-500 hover:shadow-teal-500/25 border-0" : "hover:shadow-primary/25"}`}
                  disabled={isLoading}
                  data-testid="button-submit-register"
                >
                  {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                  {isLoading ? "Creating account…" : isTeacher ? "Create Teacher Account →" : "Create Account"}
                </Button>
              </form>
            </Form>
