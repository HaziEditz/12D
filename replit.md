...
- **Enhanced Class Chat** (`client/src/pages/school/chat.tsx`): Two tabs — "Class Chat" and "Group Chats". Class chat supports edit/delete of own messages (soft delete), date separators, themed for primary/HS age groups. Group chats: create private group chats with classmates, view members, send/edit/delete messages. Real-time polling every 3s.
- **New DB Tables**: `class_group_chats` (id, classId, name, createdById), `class_group_chat_members` (chatId, userId), `class_group_chat_messages` (chatId, senderId, content, isDeleted, editedAt). Added `isDeleted` + `editedAt` to `class_group_messages`.
- **Chat API Routes**: `PUT /api/classroom/chat/:id` (edit), `DELETE /api/classroom/chat/:id` (soft-delete), `GET/POST /api/group-chats`, `GET/POST/DELETE /api/group-chats/:id/members`, `GET/POST /api/group-chats/:id/messages`, `PUT/DELETE /api/group-chats/:id/messages/:msgId`.
